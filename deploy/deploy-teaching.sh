...
# Teaching VM Deployment Script
# Deploys services to 192.168.10.29 (FLL Kids Teaching Environment)
# Usage: ./deploy-teaching.sh [service_name|all]
...
TEACHING_DIR="$REPO_DIR/teaching"
BACKUP_DIR="${BACKUP_DIR:-$REPO_DIR/.backups/teaching}"
LOG_FILE="${LOG_FILE:-$REPO_DIR/deploy/deploy-teaching.log}"
SERVICE="${1:-all}"
...
create_backup() {
    ...
    log_info "Creating backup for teaching/$service..."
    mkdir -p "$backup_path"

    # Backup docker-compose state
    if [ -f "$TEACHING_DIR/${SERVICES[$service]}/docker-compose.yaml" ] || [ -f "$TEACHING_DIR/${SERVICES[$service]}/docker-compose.yml" ]; then
        compose_file=$(ls "$TEACHING_DIR/${SERVICES[$service]}/docker-compose.y"*"ml" 2>/dev/null | head -1)
...
is_service_running() {
    ...
    if [ ! -d "$TEACHING_DIR/$service_dir" ]; then
        return 1
    fi

    cd "$TEACHING_DIR/$service_dir"
...
deploy_service() {
    ...
    log_info "Deploying teaching/$service..."

    if [ ! -d "$TEACHING_DIR/$service_dir" ]; then
        log_error "Service directory not found: teaching/$service_dir"
        return 1
    fi
    ...
    if [ ! -f docker-compose.yaml ] && [ ! -f docker-compose.yml ]; then
        log_error "No docker-compose file found in teaching/$service_dir"
        return 1
    fi
    ...
    if docker compose -f "$compose_file" ps | grep -q "Up"; then
        log_success "Successfully deployed teaching/$service"
        docker compose -f "$compose_file" ps
        return 0
    else
        log_error "Failed to deploy teaching/$service"
...
deploy_all() {
    ...
    if [ $failed -eq 0 ]; then
        log_success "All teaching VM services deployed successfully!"
        return 0
    else
...
main() {
    log_info "=================================================="
    log_info "Teaching VM Deployment (192.168.10.29)"
    log_info "=================================================="
...
# Show usage
if [ "$1" == "-h" ] || [ "$1" == "--help" ]; then
    echo "Teaching VM Deployment Script"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  $0 all          # Deploy all teaching VM services"
    echo "  $0 traefik      # Deploy only Traefik"
    echo "  $0 coder        # Deploy only Coder"
    exit 0
fi
